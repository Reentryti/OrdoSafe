<template>
  <div class="auth-page pharmacist">
    <div class="auth-container">
      <div class="auth-card">
        <div class="auth-header pharmacist">
          <div class="auth-icon">&#128138;</div>
          <h2>Inscription Pharmacien</h2>
        ...
        <div class="auth-body">
          ...
          <form @submit.prevent="handleSubmit">
            ...
            <div class="form-row">
              ...
              <div class="form-group">
                ...
                <input v-model="form.pharmacy_name" type="text" required />
              </div>
            </div>
          ...
          <div class="auth-footer">
            <p>Déjà un compte ? <router-link to="/login/pharmacist">Se connecter</router-link></p>
...
const form = reactive({
  ...
  licence_number: "", pharmacy_name: "", password1: "", password2: "", two_factor_method: "email",
});
...
async function handleSubmit() {
  ...
  try {
    ...
    const data = await signup("pharmacist", form);
    setUser(data);
...
.auth-page.pharmacist { background: linear-gradient(135deg, #43e97b, #38f9d7); }
.auth-container { width: 100%; max-width: 540px; }
.auth-card { background: rgba(255,255,255,0.97); border-radius: 20px; overflow: hidden; box-shadow: 0 20px 50px rgba(0,0,0,0.15); }
.auth-header { padding: 2rem; text-align: center; color: #fff; }
.auth-header.pharmacist { background: linear-gradient(135deg, #1cc88a, #13855c); }
...
.section-title { font-size: 0.95rem; font-weight: 700; color: #1cc88a; margin: 1.25rem 0 0.75rem; border-bottom: 1px solid #eee; padding-bottom: 0.5rem; }
.form-row { display: grid; grid-template-columns: 1fr 1fr; gap: 0.75rem; }
.form-group { margin-bottom: 0.75rem; }
.form-group label { display: block; font-weight: 600; color: #333; margin-bottom: 0.3rem; font-size: 0.85rem; }
.form-group input, .form-group select { width: 100%; padding: 0.65rem 0.8rem; border: 2px solid #e0e0e0; border-radius: 8px; font-size: 0.95rem; transition: border-color 0.2s; }
.form-group input:focus, .form-group select:focus { outline: none; border-color: #1cc88a; }
.btn-submit { width: 100%; padding: 0.85rem; background: linear-gradient(135deg, #1cc88a, #13855c); color: #fff; border: none; border-radius: 10px; font-size: 1rem; font-weight: 600; cursor: pointer; margin-top: 0.75rem; }
...
.auth-footer a { color: #1cc88a; text-decoration: none; font-weight: 600; }
